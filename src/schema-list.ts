...
import { Slice, Fragment, NodeRange, Node as ProsemirrorNode, NodeSpec, NodeType } from "prosemirror-model";
...
function add(obj: { [key: string]: any }, props: { [key: string]: any }) {
  let copy = {};
...
// `"block"`.
export function addListNodes(nodes: OrderedMap<NodeSpec>, itemContent: string, listGroup?: string) {
  return nodes.append({
...
export function wrapInList(listType: NodeType, attrs?: { [key: string]: any }) {
  return function (state: EditorState, dispatch?: (tr: Transaction) => boolean) {
    ...
    if (
      range.depth >= 2 &&
      ($from.node(range.depth - 1).type as any).compatibleContent(listType) &&
      range.startIndex == 0
    ) {
...
function doWrapInList(tr: Transaction, range: NodeRange, wrappers: any, joinBefore: any, listType: NodeType) {
  ...
  return function (state: EditorState, dispatch?: (tr: Transaction) => boolean) {
    ...
    if ($from.parent.content.size == 0) {
      ...
      if (dispatch) {
        ...
        tr.setSelection((state.selection.constructor as any).near(tr.doc.resolve($from.pos + (keepItem ? 3 : 2))));
...
function liftOutOfList(state: EditorState, dispatch?: (tr: Transaction) => boolean, range?: NodeRange) {
  let tr = state.tr,
    list = range.parent;
